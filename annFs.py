...
def train_ann(ann, inputs, outputs):
    ...
    # callback = SaverCallback("ann_training/weights", True)
    # ModelCheckpoint('weights.epoch{epoch:02d}.hdf5', monitor='val_loss', verbose=0, save_best_only=False, mode='auto')
    # if something ann.load_wieghts(filename)

    # obucavanje neuronske mreze
    # ann.fit(inputs, outputs, nb_epoch=500, batch_size=1, verbose=0, shuffle=False, show_accuracy=False, callbacks=[callback])
...
def think_and_decide(ann, img_bin):
...
class SaverCallback(ModelCheckpoint):
    def __init__(self, filepath):
        ...
        self.extension = '.h5'
        self.filepath = filepath + self.extension
        self.epoch_filepath = filepath + '.epoch{epoch:02d}' + self.extension

    def on_epoch_begin(self, epoch, logs={}):
        filepath = self.epoch_filepath.format(epoch=epoch)
